...
;(function(){
    ...
    var AlloyCrop = function (option) {
        ...
        // 图片充满裁剪框设定开关
        // 若该值为true 设定图片的初始缩放比例应该根据给定的裁剪宽高最大的缩放比例来
        // 即图片充满裁剪框
        this.imgCoverOn = option.imgCoverOn;
    ...
    AlloyCrop.prototype = {
        init: function () {
            ...
            if(this.imgCoverOn){ // 若图片充满裁剪框开关开启 则设定图片缩放比例为给定的图片的宽高相对于裁剪宽的最大比例
                var scaling_x = this.width / this.img_width,
                    scaling_y = this.height / this.img_height;
                var scaling = scaling_x > scaling_y ? scaling_x : scaling_y;
